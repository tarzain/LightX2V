"""
Modal deployment using the **official Lightricks LTX-2 non-distilled (dev) pipeline**.

Features:
- GPU memory snapshotting for fast cold starts
- CFG guidance with configurable guidance scale
- 30-step inference (configurable) at full resolution
...
WEBSOCKET_ENDPOINT = "wss://tmalive--ltx2-nondistilled-nondistilledltx2engine-streaming-app.modal.run/ws/stream"
...
app = modal.App("ltx2-nondistilled", image=image)
...
class NondistilledLTX2Engine:
    ...
    def load_model(self):
        ...
        # Create the non-distilled pipeline (single stage, CFG guidance)
    ...
    def _warmup(self):
        ...
        print(f"   Warmup: {warmup_height}x{warmup_width}, {warmup_frames} frames ({self.num_inference_steps} steps)...")

        with torch.inference_mode():
            # Use non-distilled pipeline for warmup
    ...
    def generate_streaming(
        ...
        with torch.inference_mode():
            # Pre-encode text (positive and negative prompts)
            text_encoder = self._text_encoder
            video_encoder = self._video_encoder
            video_decoder = self._video_decoder
            transformer = self._transformer

            print(f"   Streaming: Encoding prompts (CFG guidance)...", flush=True)
            print(f"   Streaming: Prompt: {prompt[:60]}...", flush=True)
            context_p, context_n = encode_text(text_encoder, prompts=[prompt, self.negative_prompt])
            ...
            if end_frame_latent is not None:
                # Calculate target frame index based on position
                ...
                print(f"   Streaming: Adding target-frame conditioning at frame {target_frame_idx}/{segment_frames-1} (pos={target_frame_position:.2f}), shape: {target_latent.shape}", flush=True)
